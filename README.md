...
| `AWS_SES_ACCESS_KEY_ID` + `AWS_SES_SECRET_ACCESS_KEY` + `AWS_SES_REGION` | Emails via AWS SES (SMTP credentials): a branded confirmation to the customer + `ORDER_NOTIFY_EMAIL` notification to the shop. The sender defaults to `ORDER_NOTIFY_EMAIL` and must be a verified SES identity; set `ORDER_EMAIL_FROM` for a custom address. Sandbox accounts only deliver to verified addresses — request SES production access for real customers |
